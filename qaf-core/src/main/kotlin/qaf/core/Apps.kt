...
object App : LocalizableWrapper() {
    ...
    val home: String by lazy {
        val path = System.getProperty(HOME_PATH_KEY)
        if (!path.isNullOrEmpty()) path else "${System.getProperty("user.home")}/.${assembly.name}"
    }
        ...
        input.bufferedReader()
                ...
                .forEach { path ->
                    try {
                        ...
                        Log.e(TAG, "not found plugin in \"{1}\"", path)
                    } catch (e: Throwable) {
    ...
    fun run(name: String, version: String, delegate: AppDelegate, arguments: Array<String>) {
        ...
        assembly = Assembly(name, version)
